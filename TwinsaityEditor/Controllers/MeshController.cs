...
namespace TwinsanityEditor
{
    public class MeshController : ItemController
    {
        ...
        public void LoadMeshData(TextureController[] tex)
        {
            if (IsLoaded) return;
            List<Vertex> vtx = new List<Vertex>();
            List<uint> idx = new List<uint>();
            ...
            for (int s = 0; s < Data.SubModels.Count; s++)
            {
                foreach (var g in Data.SubModels[s].Groups)
                {
                    if (g.VertHead > 0 && g.VDataHead > 0 && g.VertexCount >= 3)
                    {
                        ...
                        if (g.UV != null)
                        {
                            vtx.Add(new Vertex(new Vector3(-g.Vertex[0].X, g.Vertex[0].Y, g.Vertex[0].Z), new Vector2(g.UV[0].X, g.UV[0].Y), color1));
                            vtx.Add(new Vertex(new Vector3(-g.Vertex[1].X, g.Vertex[1].Y, g.Vertex[1].Z), new Vector2(g.UV[1].X, g.UV[1].Y), color2));
                        }
                        else
                        {
                            vtx.Add(new Vertex(new Vector3(-g.Vertex[0].X, g.Vertex[0].Y, g.Vertex[0].Z), color1));
                        ...
                        for (int i = 2; i < g.VertexCount; ++i)
                        {
                            ...
                            if (g.UV != null)
                            {
                                vtx.Add(new Vertex(new Vector3(-g.Vertex[i].X, g.Vertex[i].Y, g.Vertex[i].Z), new Vector2(g.UV[i].X, g.UV[i].Y), targetColor));
                            }
                            else
                            {
                                vtx.Add(new Vertex(new Vector3(-g.Vertex[i].X, g.Vertex[i].Y, g.Vertex[i].Z), targetColor));
